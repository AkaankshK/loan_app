...
//
        ...
        SizedBox(
          ...
          width: MediaQuery.of(context).size.width,
          child:
          (getLoans(loansagainst, min: min, max: max,mmin: mmin,mmax: mmax).isEmpty)
              ? Center(child: Text("Nothing to show")) :
          ListView.builder(
              itemCount: getLoans(loansagainst, min: min, max: max,mmin: mmin,mmax: mmax).length,
              itemBuilder: (_, int index) {
                var item = getLoans(loansagainst, min: min, max: max,mmin: mmin,mmax: mmax)[index];
                return Card(
                  elevation: 10,
